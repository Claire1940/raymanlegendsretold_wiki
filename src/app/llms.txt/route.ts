...
export async function GET() {
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://raymanlegendsretold.wiki'
  const siteName = process.env.NEXT_PUBLIC_SITE_NAME || 'Rayman Legends Retold Wiki'

  const lines: string[] = []
  lines.push(`# ${siteName}`)
  lines.push('')
  lines.push(`> ${siteName} is a fan-made game wiki focused on release details, editions, co-op features, trailers, and platform coverage.`)
  lines.push('')
